 ...
 * @param props
 */
export async function saveOptOutUser(props: SaveOptOutUserProps): Promise<CookieConsentRecord | null> {
    try {
...
interface CookieConsentRow extends Omit<CookieConsentRecord, 'preferences' | 'changes' | 'gpc'>, RowDataPacket {
    preferences: string;
    changes: string;
    gpc: number;
}
